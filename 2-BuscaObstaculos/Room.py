...
class Room(Environment):
    ...
    def __init__(
        self, room=[], initial_position=[0, 0], target=[], prob=0.3, n=10, plot_on=False
    ):
    ...
    def get_neighbors(self, position):
        ...
        return [
            n
            for n in neighbors
            if ((0 <= n[0] < self.room.shape[0]) and (0 <= n[1] < self.room.shape[1]))
            and (self.room[n[0]][n[1]] == 0)
        ]
    ...
    def plot_path(self, path, marker_string="-o"):
        ...
        lines = self.ax.plot(
            y, -x + len(self.room[0]) - 1, marker_string, markersize=10
        )
    ...
    def plot_position(self, pos, marker_string="ro"):
        self.ax.plot(
            pos[1], len(self.room[0]) - 1 - pos[0], marker_string, markersize=10
        )
        self.fig.canvas.draw()
