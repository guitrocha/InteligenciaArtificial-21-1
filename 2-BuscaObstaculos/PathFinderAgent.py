...
class RandAgent(Agent):
    """
    This class implements an agent that explores the environment randomly
    ...
    def run(self):
        ...
        # Run agent
        while (
            self.percepts["current_position"] != self.percepts["target"]
        ).any() and self.frontier:
            self.act()
...
class AStarAgent(PathFinderAgent):
    ...
    def remove_from_frontier(self):
        return heap.heappop(
            self.frontier
        ).get_path()  # recupero apenas o caminho, o valor de f não preciso mais
...
class BBAgent(PathFinderAgent):
    ...
    def visit_neighbours(self, path):
        ...
        last_path_cost = (
            self._last_cost
        )  # custo do caminho que foi retirado da fronteira
        ...
        if (
            last_path_cost + distance.euclidean(path[-1], self.percepts["target"])
            < self.bound
        ):
            # Visit the last node in the path
            action = {"visit_position": path[-1], "path": path}
    ...
    def update_frontier(self, path, viable_neighbors):
        ...
        if viable_neighbors:
            for neighbor in viable_neighbors:
                ...
                self.frontier = [path + [neighbor]] + self.frontier
                self.cost = [
                    last_path_bost + distance.euclidean(path[-1], [neighbor])
                ] + self.cost
